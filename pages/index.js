        ...
        <div>
          <div style={{background:'white',border:'1px solid var(--border)',padding:14,marginBottom:12}}>
            ...
            <div style={{fontSize:11,lineHeight:1.8}}>
              {[
                {label:'📱 Text — Warm', body:`Hey [Name]! It's Bri from Estēm Realty — it's been a while! Are your real estate plans still on the radar? No pressure 😊`},
                {label:'📱 Text — Direct', body:`Hi [Name], Bri Wesley with Estēm Realty. Just checking in — still thinking about buying/selling?`},
                {label:'📱 Text — Soft Follow-up', body:`Hey [Name], Bri here — just following up. No pressure at all, just want to make sure you have what you need.`},
                {label:'📞 Call — Warm', body:`Hi [Name], this is Bri Wesley with Estēm Realty. Did I catch you at a good time? Has anything changed with your real estate plans?`},
                {label:'📞 Call — Soft', body:`Hi [Name], Bri here from Estēm Realty. Not calling to pitch anything — just genuinely wanted to check in. How are things?`},
                {label:'🔔 Voicemail', body:`Hi [Name], it's Bri Wesley with Estēm Realty — just thinking about you. Call or text me when you get a chance. No rush!`},
              ].map(({label,body})=>(
                <ISAScriptCard key={label} label={label} body={body} />
              ))}
            </div>
          </div>
          <div style={{background:'white',border:'1px solid var(--border)',padding:14}}>
      ...
      <div style={{background:'white',width:'100%',maxWidth:900,padding:22,position:'relative',margin:'auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14}}>
          ...
          <div>
            ...
            <Section style={{marginTop:10}}>Script Library</Section>
...
// ── ISA SCRIPT CARD ───────────────────────────────────────────────────────────
function ISAScriptCard({label, body}) {
  ...
  return (
    <div style={{background:'var(--cream2)',border:'1px solid var(--border)',padding:'8px 10px',marginBottom:7}}>
      ...
      <button onClick={()=>{navigator.clipboard.writeText(body);setCopied(true);setTimeout(()=>setCopied(false),2000)}}
        style={{fontSize:9,padding:'2px 8px',background:copied?'var(--green)':'var(--black)',color:'white',border:'none',fontFamily:'Georgia,serif',cursor:'pointer'}}>
        {copied?'✓ Copied':'Copy'}
      </button>
    </div>
  )
  ...
  return (
    <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:8,alignItems:'center'}}>
      <a href={`tel:${e164||phone}`} style={{textDecoration:'none'}}>
        <button style={{background:'var(--green)',color:'white',border:'none',padding:'5px 12px',fontSize:10,fontFamily:'Georgia,serif',cursor:'pointer'}}>📞 Call</button>
      </a>
      <a href={`sms:${e164||phone}`} style={{textDecoration:'none'}}>
        <button style={{background:'var(--blue)',color:'white',border:'none',padding:'5px 12px',fontSize:10,fontFamily:'Georgia,serif',cursor:'pointer'}}>💬 Text</button>
      </a>
      <button onClick={()=>{navigator.clipboard.writeText(phone);setCopied(true);setTimeout(()=>setCopied(false),2000)}}
        style={{background:copied?'var(--green)':'white',color:copied?'white':'var(--black)',border:'1px solid var(--border2)',padding:'5px 12px',fontSize:10,fontFamily:'Georgia,serif',cursor:'pointer'}}>
...
function ScriptLibrary({lead, setNoteText}) {
  ...
  const select = s => { setActive(s.label); setEditText(s.body); setCopied(false) }
        ...
        <div key={cat} style={{marginBottom:9}}>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
            {scripts.filter(s=>s.cat===cat).map(s=>(
              <button key={s.label} onClick={()=>select(s)}
                style={{fontSize:9,padding:'3px 8px',border:'1px solid var(--border2)',background:active===s.label?'var(--black)':'white',color:active===s.label?'white':'var(--gray)',fontFamily:'Georgia,serif',cursor:'pointer'}}>
                {s.label}
              </button>
            ))}
          </div>
      ...
      {editText && (
        <div style={{marginTop:8}}>
          ...
          <div style={{display:'flex',gap:5,marginTop:5}}>
            <button onClick={()=>{navigator.clipboard.writeText(editText);setCopied(true);setTimeout(()=>setCopied(false),2000)}}
              style={{background:copied?'var(--green)':'var(--black)',color:'white',border:'none',padding:'5px 12px',fontSize:10,fontFamily:'Georgia,serif',cursor:'pointer'}}>
              {copied?'✓ Copied!':'📋 Copy Script'}
            </button>
            <button onClick={()=>setNoteText&&setNoteText(editText)}
              style={{background:'white',border:'1px solid var(--border2)',color:'var(--black)',padding:'5px 12px',fontSize:10,fontFamily:'Georgia,serif',cursor:'pointer'}}>
              📝 Use as Note
            </button>
          </div>
        </div>
